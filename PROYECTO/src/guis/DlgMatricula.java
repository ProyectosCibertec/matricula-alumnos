...
WindowListener, KeyListener, MouseListener {
    ...
    protected void actionPerformedButton_1(ActionEvent arg0) {
        switch (tipoOperacion) {
	                ...
	                if (ArregloAlumno.buscar(Integer.parseInt(textField_1.getText())) != null) {
	                    ...
	                    if (ArregloCurso.buscar(Integer.parseInt(textField_2.getText())) != null) {
	                        ...
	                        if (ArregloAlumno.buscar(Integer.parseInt(textField_1.getText())).getEstado() != 1) {
	                        	try{
		                            try {
		                            	int condicion = JOptionPane.showConfirmDialog(this, "Desea matricular a este alumno?", "Confirmación", 0,1,null);
		                            	if(condicion == 0){
            ...
            case 2:
                if (ArregloCurso.buscar(Integer.parseInt(textField_2.getText())) != null) {
                	int condicion = validar("Desea modificar esta matrícula?", "Confirmación");
                	if(condicion == 0){
            ...
            case 3:
                if (ArregloAlumno.buscar(Integer.parseInt(textField_1.getText())).getEstado() != 2) {
                    ...
                    error("El alumno se encuentra actualmente retirado de la institución.");
                }
                break;
        }
    }
		...
		try{
			if(arg0.getKeyCode() == KeyEvent.VK_ENTER){
				try{
					...
					error("Este código de alumno no existe");
				}
			}
		}
		catch(Exception e){
			error("Ingrese un código de alumno válido");
		...
		try{
			if(arg0.getKeyCode() == KeyEvent.VK_ENTER){
				try{
					...
					error("Este código de curso no existe");
				}
			}
		}
		catch(Exception e){
			error("Ingrese un código de curso válido");
		}
	}
