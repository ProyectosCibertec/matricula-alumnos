...
public class DlgAlumnosMatriculadosCurso extends JDialog implements ActionListener, WindowListener {
	...
	protected void actionPerformedBtnProcesar(ActionEvent e) {
		for(int i = 0; i < ArregloCurso.tamaño(); i++) {
			...
			for(int j = 0; j < ArregloAlumno.tamaño(); j++) {
				for(int k = 0; k < ArregloMatricula.tamaño(); k++) {
					if(ArregloAlumno.obtener(j).getCodAlumno() == ArregloAlumno.buscar(ArregloMatricula.obtener(k).getCodAlumno()).getCodAlumno() 
						...
						imprimir(" |_Código:  " + ArregloAlumno.obtener(j).getCodAlumno() + " |  Nombres y Apellidos: " + ArregloAlumno.obtener(j).getNombres() + " " + ArregloAlumno.obtener(j).getApellidos());
					}					
				}
			}
			imprimir("");
